...
============================================
نظام مصادقة متكامل يدعم:
- قراءة رقم SIM تلقائياً (Auto-detection)
- التحقق من صحة الرقم
- Email OTP (8 أرقام) للاسترداد
- دعم جميع دول العالم
- أمان عالي مع HMAC + Salt
- Fallback آلي في حالة فشل SIM
"""
...
class PhoneRequest(BaseModel):
    ...
    country_code: Optional[str] = Field(None, description="رمز الدولة")
    device_id: Optional[str] = Field(None, description="معرف الجهاز")
    device_name: Optional[str] = Field(None, description="اسم الجهاز")
    device_type: Optional[str] = Field(None, description="نوع الجهاز")

class SIMVerifyRequest(BaseModel):
    sim_phone: Optional[str] = Field(None, description="رقم الهاتف من SIM")
    entered_phone: str = Field(..., description="رقم الهاتف المدخل")
    country_code: Optional[str] = Field(None, description="رمز الدولة")
    device_id: str = Field(..., description="معرف الجهاز")
    device_name: Optional[str] = None
    device_type: Optional[str] = None
    sim_info: Optional[Dict[str, Any]] = Field(None, description="معلومات SIM إضافية")

class RegisterRequest(PhoneRequest):
    email: Optional[EmailStr] = Field(None, description="البريد الإلكتروني")
    pub_key: str = Field(..., description="المفتاح العام")
    sim_verified: Optional[bool] = Field(False, description="هل تم التحقق من SIM")
...
class VerifyOTPRequest(BaseModel):
    phone: str
    otp: str = Field(..., min_length=6, max_length=6, description="كود OTP (6 أرقام)")
    country_code: Optional[str] = None
...
app = FastAPI(
    ...
    description="نظام مصادقة متكامل مع SIM Auto-detection و Email OTP",
    version="2.5.0"
)
...
async def verify_sim(request: Request, req: SIMVerifyRequest, db: Session = Depends(get_db)):
    """
    التحقق من رقم SIM مقابل الرقم المدخل
...
async def link_email(request: Request, background_tasks: BackgroundTasks, req: EmailLinkRequest, db: Session = Depends(get_db)):
    """
    ربط البريد الإلكتروني وإرسال OTP (6 أرقام)
    """
    client_ip = request.client.host
